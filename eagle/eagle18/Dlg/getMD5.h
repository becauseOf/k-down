...
CString getMd5(CString  FilePathName)
{
	FILE *fp;
	/*const size_t strsize=(FilePathName.GetLength()+1)*2; // 宽字符的长度;
	char * pstr= new char[strsize]; //分配空间;
	size_t sz=0;
	wcstombs_s(&sz,pstr,strsize,FilePathName,_TRUNCATE);
	atoi((const char*)pstr); // 字符串已经由原来的CString 转换成了 const char*
	//const char* filename =FilePathName.GetBuffer(sizeof(FilePathName));*/
	fp=_wfopen(FilePathName,L"rb");//以二进制打开文件
	...
	cstri1.Format(_T("%08x"),PP(A));
	cstri2.Format(_T("%08x"),PP(B));
	cstri3.Format(_T("%08x"),PP(C));
	cstri4.Format(_T("%08x"),PP(D));
