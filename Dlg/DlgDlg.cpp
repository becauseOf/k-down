...
BOOL CDlgDlg::OnInitDialog()
{
	...
	//为任务列表添加行头
	m_listShow.InsertColumn(1,_T("文件名"),0,290,-1);
	m_listShow.InsertColumn(2,_T("文件大小"),0,145,-1);
	...
	m_bDownloading = false;
	//获取默认保存的路径
	GetPrivateProfileString(_T("DownloadPath"),_T("DownloadPath"),_T("D:\\"),m_csSaveFilePath.GetBuffer(MAX_PATH),MAX_PATH,_T("..\\DownloadPath.xiangsong"));
	m_bFinish = false;
	UpdateData(false);
...
void CDlgDlg::OnBnClickedButtonSetip()
{
	...
	GetIP(m_csIP);//获取本机IP地址
	m_csMAC=GetMAC();//获取网卡MAC地址
...
void CDlgDlg::DealDownload(CString FileName,CString MD5,int iFileSize)
{
	...
	m_pSendSocket->Send((char*)&downloadMsg,sizeof(downloadMsg)+1);
	//添加到任务列表
	int iRow = m_listShow.GetItemCount();
	//添加列表
	CString csFileSize;
	csFileSize.Format(_T("%d"),iFileSize);
	m_listShow.InsertItem(iRow,FileName);
	m_listShow.SetItemText(iRow,1,csFileSize);
